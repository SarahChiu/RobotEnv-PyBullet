...
class KukaContiStackInHandEnv(gym.Env):
  ...
  def getMedInitState(self):
    self.reset()
    medJointPos=[ 0.006418, 0.785398, -0.011401, -1.589317, 0.005379, 0.785398, -0.006539, \
      0.000048, -0.299912, 0.000000, -0.000043, 0.299960, 0.000000, -0.000200 ]
    self._kuka.initState(medJointPos, self._renders)
    self._observation = self.getExtendedObservation()
  ...
  def _reward(self):
    ...
    blockPos,blockOrn=p.getBasePositionAndOrientation(self.block1Uid)
