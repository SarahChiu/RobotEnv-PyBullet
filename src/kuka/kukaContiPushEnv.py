...
class KukaContiPushEnv(KukaContiEnv):
  def __init__(self, renders=False):
    super(KukaContiPushEnv, self).__init__(renders=renders)
  ...
  def reset(self):
    ...
    xpos = np.clip(endEffectorPos[0] + 0.075*random.choice([-1,1]) + 0.05*random.random(), 0.455, 0.69)
    ypos = np.clip(endEffectorPos[1] + 0.075*random.choice([-1,1]) + 0.05*random.random(), -0.125, 0.25)
  ...
  def _termination(self):
    ...
    if (np.linalg.norm([blockPos[0]-0.5725, blockPos[1]-0.0625])<=0.01 or self._envStepCounter > 10):
      self.terminated = 1
      self._observation = self.getExtendedObservation()
      return True
  ...
  def _reward(self):
    ...
    #rewards is the xy pos of the target object
    blockPos,_=p.getBasePositionAndOrientation(self.blockUid)

    reward = 0.0

    if (np.linalg.norm([blockPos[0]-0.5725, blockPos[1]-0.0625])<=0.01 and self.terminated):
      reward = 1.0
